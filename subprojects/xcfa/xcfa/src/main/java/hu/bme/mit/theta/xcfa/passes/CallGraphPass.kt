...
class CallGraphPass : ProcedurePass {
    override fun run(builder: XcfaProcedureBuilder): XcfaProcedureBuilder {
        ...
        builder.getEdges().map { it.getFlatLabels().filter { it is InvokeLabel || it is StartLabel  } }.flatten().forEach {
            when(it) {
        ...
        builder.parent.getProcedures().filter { calledProcedures.contains(it.name) }.forEach { it.metaData["reachable-from-${builder.name}"] = Unit }
        return builder
    }
}